...
const tokens = (n) => { // Helper that converts currency to token
...
describe('Escrow', () => {
    ...
    beforeEach(async () => {
        ...
        // Assume the first minted token has ID 0
        tokenId = 0;
        ...
        // List Property
        transaction = await escrow.connect(seller).list(tokenId, buyer.address, tokens(10), tokens(5))
        await transaction.wait()
    })
    ...
    describe('Listing', () => {
        it('Updates as listed', async () => {
            const result = await escrow.isListed(tokenId)
            expect(result).to.be.equal(true)
        })

        it('Returns buyer', async () => {
            const result = await escrow.buyer(tokenId)
            expect(result).to.be.equal(buyer.address)
        })

        it('Returns purchase price', async () => {
            const result = await escrow.purchasePrice(tokenId)
            expect(result).to.be.equal(tokens(10))
        })

        it('Returns escrow amount', async () => {
            const result = await escrow.escrowAmount(tokenId)
            expect(result).to.be.equal(tokens(5))
        })

        it('Updates ownership', async () => {
            expect(await realEstate.ownerOf(tokenId)).to.be.equal(escrow.address)
        })
    })

    describe('Deposits', () => {
        beforeEach(async () => {
            const transaction = await escrow.connect(buyer).depositEarnest(tokenId, { value: tokens(5) })
            await transaction.wait()
        })
    ...
    describe('Inspection', () => {
        ...
        it('Updates inspection status', async () => {
            const result = await escrow.inspectionPassed(tokenId)
            expect(result).to.be.equal(true)
        })
    })
    ...
    describe('Sale', () => {
        beforeEach(async () => {
            let transaction = await escrow.connect(buyer).depositEarnest(tokenId, { value: tokens(5) })
            await transaction.wait()
        ...
        it('Updates ownership', async () => {
            expect(await realEstate.ownerOf(tokenId)).to.be.equal(buyer.address)
        })
